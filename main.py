...
async def take_screenshot(post_info_list):
    ...
    try:
        ex = 0
        while ex != len(post_info_list):
            browser.get(post_info_list[ex])
            browser.execute_script("window.scrollTo(0, document.body.scrollHeight);")
            screenshot_path = f'post_comment_{random.randrange(0,100000)}_{ex}_{random.randint(0,1000000)}.png'
...
async def main():
    ...
    while ex != len(post_info_list):
        ...
        match = re.search(r'vk\.com/wall-(\d+)_(\d+)', post_info_list[ex])
